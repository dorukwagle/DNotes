...
public class CollectionModel implements IModel<CollectionDto> {
    ...
    public void softDelete(String id) {
        try {
            ...
            var books = connection.prepareStatement("update books set deletedAt = CURRENT_TIMESTAMP WHERE collectionId = ?");
            books.setString(1, id);
            books.executeUpdate();

            var bookPages = connection.prepareStatement(
                    "update bookPages set deletedAt = CURRENT_TIMESTAMP WHERE bookId IN (SELECT id FROM books WHERE collectionId = ?)");
    ...
    public List<CollectionDto> getAll(PaginationParams params) {
        try (var stmt = new PaginateQuery("collectionView", params)
