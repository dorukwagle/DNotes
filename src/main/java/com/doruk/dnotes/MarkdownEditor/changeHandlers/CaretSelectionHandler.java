...
public class CaretSelectionHandler {
    ...
    private static final Duration DELAY = Duration.ofMillis(400);
        ...
        allTools.forEach(tuple -> {
            ...
            switch (ToolName.fromName(tuple.getKey())) {
                case FontColor -> {
                    ...
                    controlPanel.getTextColorPicker().setValue(color);
                ...
                case FontBG -> {
                    var color = (Color) stateTool.getState();
                    controlPanel.getHighColorPicker().setValue(color);

                    if (GlobalConstants.DEFAULT_FONT_BG_COLOR.equals(color))
                        btnMap.get(tuple.getKey()).setSelected(false);
                }
                case Font -> {
                    controlPanel.getFontSizeCombo().setValue(stateTool.getState().toString());
                }
                default -> {
                    return;
                }
