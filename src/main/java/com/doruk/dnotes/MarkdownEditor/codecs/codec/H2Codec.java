...
public class H2Codec extends Codec<ParagraphNode, ParagraphStyle, MutableParagraphStyle> {
    @Override
    ...
    public void encode(ParagraphNode node, ParagraphStyle style) {
        if (!this.isApplied(style, ParagraphType.H2))
            return;

        node.addGlobalStyle(ToolName.H2);
    }

    @Override
    public void decode(ParagraphNode node, MutableParagraphStyle style) {
        if (!node.getGlobalStyles().contains(ToolName.H2))
            return;

        style.addStyle(ParagraphType.H2);
    }
}
