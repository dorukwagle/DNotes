...
public class AlignLeftCodec extends Codec<ParagraphNode, ParagraphStyle, MutableParagraphStyle> {
    @Override
    ...
    public void decode(ParagraphNode node, MutableParagraphStyle style) {
        if (node.getGlobalStyles().contains(ToolName.AlignLeft))
            style.addStyle(ParagraphType.ALIGN_LEFT);
    }
}
